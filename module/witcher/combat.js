...
function validateHands(actor, weapon) {
  ...
  if (held.reduce((sum, i) => sum + i.system.hands, 0) > 2)
    throw new RuleError('Equipped weapons and shields require more than two hands.');
  for (const w of actor.items.filter((i) => i.type === 'wound')) {
    const state = w.system.wound;
    const mods =
      state.treatment === 'treated'
        ? state.treatedModifiers
        : state.treatment === 'stabilized'
          ? state.stabilizedModifiers
          : state.modifiers;
    if (mods.armDisabled && held.reduce((sum, i) => sum + i.system.hands, 0) > 1)
      throw new RuleError('A disabled arm cannot hold a weapon.');
  }
}
...
export async function attack(actor, item, options = {}) {
  ...
  const weapon = options.weapon ?? (item ? itemSnapshot(item) : unarmed(actor, options.action ?? 'punch'));
  ...
  const fields =
    ...
    input('action', 'Action', {
      ...
      options: Object.fromEntries(SPECIAL_ACTIONS.filter((k) => k !== 'joint').map((k) => [k, titleCase(k)])),
    }) +
  ...
  return serial(actor.uuid, async () => {
    ...
    if (currentContinue && (style !== actor.system.combat.style || action !== 'normal'))
      throw new RuleError('Finish the remaining strikes in this action before changing style.');
    const profile = strikeProfile(w, {
      style,
      action,
      npc: minor,
      underwater: actor.system.environment.underwater && !actor.system.traits.amphibious,
    });
    const plan = currentContinue
      ? { changes: {}, modifier: actor.system.combat.extraPenalty, cost: 0 }
      : actionPlan(actor, {
          extra: values.extra,
          full: action === 'charge' || w.properties?.fullRound,
          recovery: action === 'escape',
        });
    ...
    if (RANGED.includes(w.category)) {
      ...
      range = rangeBracket(Number(values.distance), rangeValue);
    ...
    if (['bow', 'crossbow'].includes(w.category)) {
      ...
      if (!ammo?.system.isAmmo || ammo.system.quantity < 1)
    ...
    changes['system.combat.remaining'] = currentContinue
      ? actor.system.combat.remaining - 1
      : profile.attacks - 1;
    changes['system.combat.weaponId'] = w.id;
    changes['system.combat.style'] = style;
    changes['system.combat.extraPenalty'] = plan.modifier;
    ...
    return chat(
      ...
      checkHTML(result) +
        ...
        `<button type="button" data-witcher-action="defend">Defend / resolve</button>`,
...
export async function defend(message) {
  ...
  const equipment = actor.items.filter(
    (i) => i.system.equipped && ['weapon', 'shield'].includes(i.type) && i.system.reliability > 0
  );
  const options = {
    dodge: 'Dodge / Escape',
  ...
  if (
    actor.items.some((i) => i.type === 'ability' && i.system.ability.key === 'shift') &&
    !suppressed(actor.system, 'Moondust') &&
    !suppressed(actor.system, 'Yrden')
  )
    options.shift = 'Shift (Spell Casting)';
  ...
  return serial(actor.uuid, async () => {
    ...
    if (armed && (!w?.system.equipped || w.system.reliability <= 0))
    ...
    const skill =
      defense === 'shift'
        ...
        : defense === 'awareness'
          ? 'awareness'
          : defense === 'grapple' || defense === 'blockArm'
            ? 'brawling'
            : defense === 'reposition'
              ? 'athletics'
              : defense === 'dodge'
    ...
    const result = passive
      ? { total: Number(values.dc), rolls: [], base: Number(values.dc), dice: [], fumble: 0 }
    ...
    return chat(
      ...
      checkHTML(result) +
        ...
        (defense === 'reposition'
          ...
          : '') +
        (result.fumble ? `<p>${e(fumbleText(armed ? 'armedDefense' : 'unarmed', result.fumble))}</p>` : '') +
...
export async function resolveDefense(message) {
  if (!isPrimaryActiveGm()) throw new RuleError('The active GM resolves attacks against other actors.');
  return serial('gm-resolve', async () => {
    ...
    if (
      !attacker.testUserPermission(attackMessage.author, 'OWNER') ||
      !target.testUserPermission(message.author, 'OWNER')
    )
    ...
    if (!hit && defense.defense !== 'blockArm') {
      ...
      if (['blockWeapon', 'blockShield'].includes(defense.defense) && w)
        await w.update({
...
export async function prepareDamage(attacker, target, a, defense = {}) {
  ...
  for (const loc of chosen) {
    const roll = await dice(w.damage || '0');
    rolls.push(roll);
    let raw = Math.max(
      0,
      roll.total +
        (w.id === 'unarmed' ||
        w.properties?.natural ||
        w.properties?.fixedDamage ||
        RANGED.includes(w.category) ||
        w.category === 'trap'
          ? 0
          : a.meleeBonus)
    );
    ...
    const request = {
      ...
      nonlethal: properties.nonlethal || a.action === 'pommel',
      cover: a.cover ?? 0,
...
export async function applyDamage(message) {
  if (!isPrimaryActiveGm()) throw new RuleError('The active GM applies damage.');
  return serial('gm-damage', async () => {
    ...
    if (data.applied || target.system.combat.applied.includes(message.id))
      throw new RuleError('This damage has already been applied.');
    if (damageState(target) !== data.state) {
      ...
      await message.update({
        ...
        content: `<article class="witcher-chat"><h3>Damage recalculated: ${e(target.name)}</h3>${damageHTML({ ...data, results })}<p>Armor or resources changed. Review the updated calculation.</p><button type="button" data-witcher-action="apply">Apply damage (GM)</button></article>`,
      });
      return;
    }
    ...
    if (data.stun !== null && data.stun !== undefined && !has(target, 'dead'))
      await save(target, 'stun', { modifier: data.stun });
    if (target.system.pendingDeathSaves && !has(target, 'dead'))
      await chat(
        target,
        'Death save required',
...
export function registerCombatChat() {
  ...
  Hooks.on('createChatMessage', (message) => {
    if (
      isPrimaryActiveGm() &&
      message.getFlag(SYSTEM_ID, 'kind') === 'defense' &&
      message.flags[SYSTEM_ID].check.fumble <= 5
    )
      resolveDefense(message).catch(errorNotice);
  });
}
