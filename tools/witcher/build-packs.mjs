...
for(const pack of manifest.packs){
  ...
  for(const item of items)await writeFile(path.join(source,item._id+'.json'),JSON.stringify({...item,_key:`!items!${item._id}`}));
  ...
  assert.equal(readback.filter(p=>p.endsWith('.json')).length,items.length,`${pack.name}: stored document count`);
  const documents=await Promise.all(readback.filter(p=>p.endsWith('.json')).map(async p=>JSON.parse(await readFile(path.join(extracted,p),'utf8'))));
  for(const item of items){const stored=documents.find(d=>d._id===item._id);assert.deepEqual(stored?.system,item.system,`${pack.name}/${item.name}: stored system data`);}
