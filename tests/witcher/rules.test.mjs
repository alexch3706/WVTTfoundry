...
test("p.57 defender wins ties, and a difficulty must be exceeded",()=>{
  assert.equal(r.beats(14,14),false);assert.equal(r.beats(15,14),true);
});
test("p.155 armor example uses augmented pair, capped bonus and exact boundaries",()=>{
  assert.equal(r.stackArmor([{sp:3,kind:"light"},{sp:12,kind:"medium"},{sp:20,kind:"heavy"}]),24);
  assert.deepEqual([4,5,8,9,14,15,20,21].map(r.armorBonus),[5,4,4,3,3,2,2,0]);
  assert.equal(r.stackArmor([{sp:1,kind:"light"},{sp:1,kind:"light"}]),2);
  assert.throws(()=>r.stackArmor([{sp:20,kind:"heavy"},{sp:30,kind:"heavy"}]));
  assert.equal(r.stackArmor([{sp:3,kind:"light"},{sp:5,kind:"light"},{sp:8,kind:"light"}]),13);
});
test("p.154 human and monster tables cover all ten rolls with different torso/limb boundaries",()=>{
  assert.equal(r.validateLocations(HUMANOID_LOCATIONS),true);assert.equal(r.validateLocations(MONSTER_LOCATIONS),true);
  ...
  assert.throws(()=>r.validateLocations([{...MONSTER_LOCATIONS[0],max:10},...MONSTER_LOCATIONS.slice(1)]));
});
const armor=(sp,kind="light",resistances=[])=>({id:"armor",type:"armor",equipped:true,armorClass:kind,coverage:["head","torso"],stoppingPower:sp,resistances});
test("p.154 armor is subtracted before hit location; penetration wears armor",()=>{
  ...
  assert.equal(result.damage,24);assert.equal(result.armorChanges[0].after,11);
  const blocked=r.resolveDamage({raw:12},{},HUMANOID_LOCATIONS[0],[armor(12)]);
  assert.equal(blocked.damage,0);assert.deepEqual(blocked.armorChanges,[]);
});
...
test("p.158 critical bonus bypasses armor without wearing an unpenetrated layer",()=>{
  ...
  assert.equal(result.damage,5);assert.equal(result.armorChanges.length,0);
});
test("p.162 silver monster resistance excludes fire; silver weapon adds listed damage",()=>{
  const target={silverVulnerable:true};const loc=MONSTER_LOCATIONS[1];
...
test("repeated location resolution is independent and does not mutate source data",()=>{
  const request={raw:20};const target={};const items=[armor(12)];const before=structuredClone({request,target,items});
  ...
  assert.equal(head.damage,24);assert.equal(torso.damage,8);
...
test("p.48 derived statistics round down and weight penalty applies once per stat",()=>{
  ...
  assert.equal(d.hpMax,25);assert.equal(d.stun,5);assert.equal(d.overweight,2);
  assert.equal(d.stats.ref,6);assert.equal(d.stats.dex,5);assert.equal(d.stats.spd,4);
});
...
test("p.151 one normal action, one extra action for 3 STA; successive defenses cost STA",()=>{
  const first=r.reserveAction({});assert.equal(first.cost,0);
  assert.throws(()=>r.reserveAction(first.budget));
  const extra=r.reserveAction(first.budget,{extra:true});assert.equal(extra.cost,3);assert.equal(extra.modifier,-3);
  assert.throws(()=>r.reserveAction(extra.budget,{extra:true}));
  const defense=r.reserveAction({},{defense:true});assert.equal(defense.cost,0);
...
test("p.164 thrown parry is -5, normal parry -3, arrows cannot be parried",()=>{
  assert.equal(r.defenseModifier("parry","thrown"),-5);assert.equal(r.defenseModifier("parry","sword"),-3);
...
test("p.164 range bands include exact limits and reject beyond extreme",()=>{
  assert.deepEqual([.5,25,50,100,200].map(d=>r.rangeBracket(d,100).modifier),[5,0,-2,-4,-6]);
