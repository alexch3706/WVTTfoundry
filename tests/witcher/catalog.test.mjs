...
test('catalog has every extracted core equipment row and stable unique IDs', async () => {
  const audit = JSON.parse(
    await fs.readFile(new URL('../../data/witcher/source-audit.json', import.meta.url), 'utf8')
  );
  assert.equal(all.length, 673);
  assert.equal(audit.rows.length, all.length);
  for (const [name, items] of Object.entries(packs)) {
    ...
    for (const item of items) {
      assert.match(item._id, /^[a-z0-9]{16}$/);
      assert.ok(ITEM_TYPES.includes(item.type));
      assert.ok(item.system.page > 0);
      assert.ok(Number.isFinite(item.system.weight));
      assert.ok(Number.isFinite(item.system.cost));
    }
  }
});
test('p.73–74: quantities, price and weight for ammunition use the same unit', () => {
  const ammo = find('Standard Ammunition');
  assert.equal(ammo.quantity, 10);
  assert.equal(ammo.weight * ammo.quantity, 0.5);
  assert.equal(ammo.cost * ammo.quantity, 10);
...
test('p.249: graphical potion recipes retain each ingredient symbol and quantity', () => {
  ...
  assert.deepEqual(
    recipe.materials.map((i) => [i.name, i.quantity]),
    [
      ['Vitriol', 3],
      ['Rebis', 1],
      ['Aether', 1],
  ...
  assert.deepEqual(
    find('Swallow Formula').materials.map((i) => i.name),
    ['Vitriol', 'Aether', 'Caelum']
  );
});
test('p.250: blade oils include dog tallow; decoctions include mutagen and spirits', () => {
  ...
  assert.equal(find('Arachas Decoction').duration, 600);
  assert.equal(find('Arachas Decoction').toxicity, 75);
});
test('all diagrams link to products; the source’s Etching Oil omission remains explicit', async () => {
  for (const recipe of packs.diagrams) {
    assert.ok(recipe.system.productUuid, recipe.name);
    assert.ok(recipe.system.materials.length, recipe.name);
  }
  const audit = JSON.parse(
    await fs.readFile(new URL('../../data/witcher/source-audit.json', import.meta.url), 'utf8')
  );
...
test('p.254: experimental weights and bomb range are not ordinary thrown-weapon values', () => {
  assert.equal(find('Dancing Star').weight, 1);
