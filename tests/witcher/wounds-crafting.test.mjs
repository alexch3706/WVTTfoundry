...
test('p.158–160: all 24 wounds and every 2d6 result resolve', () => {
  ...
  for (const severity of Object.keys(WOUNDS))
    for (let roll = 2; roll <= 12; roll++) {
      const result = criticalWound(severity, HUMANOID_LOCATIONS, { roll, side: 1 });
      assert.equal(result.wound.severity, severity);
      assert.ok(result.location.id);
    }
});
test('p.158: aimed head/torso criticals use 1–4 lesser, 5–6 greater', () => {
  assert.equal(
    criticalWound('simple', HUMANOID_LOCATIONS, { aimed: 'head', greater: 4 }).wound.name,
    'Disfiguring Scar'
  );
  assert.equal(
    criticalWound('simple', HUMANOID_LOCATIONS, { aimed: 'head', greater: 5 }).wound.name,
    'Cracked Jaw'
  );
  assert.equal(
    criticalWound('deadly', HUMANOID_LOCATIONS, { aimed: 'torso', greater: 4 }).wound.name,
    'Septic Shock'
  );
  assert.equal(
    criticalWound('deadly', HUMANOID_LOCATIONS, { aimed: 'torso', greater: 5 }).wound.name,
    'Heart Damage'
  );
  assert.equal(
    criticalWound('deadly', HUMANOID_LOCATIONS, { aimed: 'head', greater: 4, balanced: 1 }).wound.fatal,
    true
  );
});
test('p.159: organless monsters replace specified wounds with their bonus damage', () => {
  ...
  assert.equal(result.location.id, 'torso');
  assert.equal(
    criticalWound('difficult', MONSTER_LOCATIONS, { roll: 11, organless: true }).wound.name,
    'Concussion'
  );
});
...
test('p.158: rib injury changes BODY without reducing maximum HP', () => {
  ...
  assert.equal(d.stats.body, 6);
  assert.equal(d.hpMax, 35);
  items[0].wound.treatment = 'treated';
  assert.equal(derivedStats(actor, items).stats.body, 8);
  assert.equal(derivedStats(actor, items).enc, 70);
});
test('p.158: foreign object recovery progresses from quarter to half to −2', () => {
  ...
  assert.equal(derivedStats(actor, [{ type: 'wound', wound: w }]).rec, 4);
  w.treatment = 'treated';
  assert.equal(derivedStats(actor, [{ type: 'wound', wound: w }]).rec, 6);
});
test('p.23: dwarf SP does not ablate and their encumbrance increases by 25', () => {
  ...
  assert.equal(damage.naturalChange.after, 0);
...
test('p.162: meteorite exemption belongs to the specific monster', () => {
  const location = HUMANOID_LOCATIONS[1];
  assert.equal(
    resolveDamage({ raw: 10, properties: { meteorite: true } }, { silverVulnerable: true }, location).damage,
    5
  );
  assert.equal(
    resolveDamage(
      { raw: 10, properties: { meteorite: true } },
      { silverVulnerable: true, meteoriteVulnerable: true },
      location
    ).damage,
    10
  );
...
test('p.127: materials cannot be reused across repeated requirements', () => {
  ...
  assert.deepEqual(
    allocateMaterials(materials, [{ id: 'a', name: 'Iron', quantity: 5 }]).map((x) => [x.quantity, x.after]),
    [[4, 1]]
  );
});
test('p.142: alchemy can substitute ingredients containing the same substance', () => {
  const result = allocateMaterials(
    [{ name: 'Vitriol', substance: 'Vitriol', quantity: 3 }],
    [
      { id: 'a', name: 'Barley', substance: 'Vitriol', quantity: 1 },
      { id: 'b', name: 'Crow’s Eye', substance: 'Vitriol', quantity: 5 },
    ]
  );
  assert.deepEqual(
    result.map((i) => [i.id, i.quantity, i.after]),
    [
      ['a', 1, 0],
      ['b', 2, 3],
    ]
  ...
  assert.throws(
    () => allocateMaterials([{ name: 'Iron', quantity: 1 }], [{ id: 'x', name: 'Dark Iron', quantity: 5 }]),
    /Missing/
  );
});
test('p.127,142: crafting recovers half rounding up, alchemy exactly one chosen substance', () => {
  const used = [
    { id: 'a', quantity: 3, substance: 'Vitriol' },
    { id: 'b', quantity: 2, substance: 'Rebis' },
  ];
  assert.deepEqual(craftingRecovery(used, { success: true }), [
    { id: 'a', quantity: 2 },
    { id: 'b', quantity: 1 },
  ]);
  assert.deepEqual(craftingRecovery(used, { success: true, alchemy: true }), [
    { name: 'Vitriol', quantity: 1 },
    { name: 'Rebis', quantity: 1 },
  ]);
